...
import CreatePriceType from "@/components/priceType/CreatePriceType.vue";
import ViewPriceType from "@/components/priceType/ViewPriceType.vue";
import MethodsPriceType from "@/components/priceType/MethodsPriceType.vue";
...
async function getProducts(filters = {}) {
  ...
  const res = await useAxios(`priceType?search=`, {params});
...
<template>
  <header-purchase header-title="Виды цен"/>
  ...
  <div class="card mt-4 bg-white h-[75vh] overflow-auto relative bottom-[43px]">
    <MethodsPriceType  @get-product="getProductMethods" :select-products="selectedProduct"
  ...
  <div class="create-purchase-sidebar">
    ...
    >
      <view-price-type v-if="createOpenModal" :product-id="selectedProductId" @close-sidebar="closeView"
                 @close-dialog="closeFn"/>
      <CreatePriceType v-else @close-dialog="closeFn" @close-sidebar="closeView"/>
