...
import ViewStorage from "@/components/storageComponents/ViewStorage.vue";
import CreateStorage from "@/components/storageComponents/CreateStorage.vue";
...
async function getProducts(filters = {}) {
  ...
  try {
    const res = await useAxios(`/storage`, {params});
...
<template>
  <header-purchase header-title="Склады"/>
  ...
  <div class="create-purchase-sidebar">
    ...
    >
      ...
      <view-storage :product-id="dataInfo.id" v-if="createOpenModal" @close-sidebar="closeFnVl" :data="dataInfo"
                         :openModalClose="openInfoModal"/>
      <CreateStorage v-else @close-sidebar="visibleRight = false" @close-dialog="closeFn"/>
    </Sidebar>
  </div>
