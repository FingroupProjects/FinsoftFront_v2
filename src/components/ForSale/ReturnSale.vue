...
const emit = defineEmits(['close-modal', 'postReturnSale'])
...
async function getArchiveList(value) {
  try {
    const params = {
      search: value // Replace searchTerm with your actual search variable
    };
    const res = await useAxios('rmk', {params});
...
function postFn() {
  emit('postReturnSale', selectedProduct)
...
<template>
  <Dialog :draggable="false" class="fast-goods-header" v-model:visible="props.openReturnSale" modal
