...
import {useClientOrder} from "@/store/clientOrder.js";
  ...
  <div>
    <div class="edit-purchase">
      ...
      <div v-if="isOpen"
        ...
        <FloatLabel class="col-span-4"  >
          <Select
              v-model="viewDocument.orderStatusName"
              class="w-full"
              :options="statusList"
              option-label="name"
              @click="findStatus"
          >
            <template #value>
              {{ viewDocument.orderStatusName?.name }}
            </template>
          </Select>
          <label for="dd-city">Организация</label>
        </FloatLabel>
    ...
    <client-order-table :info-goods="props.data" @post-goods="getProducts" @editModal="changeModal"/>
