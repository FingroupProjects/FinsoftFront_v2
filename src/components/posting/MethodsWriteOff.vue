...
const copyProducts = async () => {
  ...
  try {
    const res = await useAxios(`/inventoryOperation/copy/${id.value[0]}`, {
      method: "POST",
    });
    emit('getProduct')
...
const deleteProduct = async () => {
  ...
  const endpoint = props.selectProducts[0].deleted_at
      ? '/inventoryOperation/massRestore'
      :'/inventoryOperation/massDelete' ;
...
async function conductMethod(){
  ...
  const endpoint = props.selectProducts[0].active
      ? '/inventoryOperation/unApprove'
      :'/inventoryOperation/approve' ;
