...
const calculateInstallments = () => {
  if (selectedTerm.value && props.allSum) {
    const installmentAmount = props.allSum / selectedTerm.value;
...
const getWeekNumber = (date) => {
  ...
  return weekDaysMap[date.getDay()] + 1;
};

const updateWeekDays = () => {
  weekDays.value = dateValues.value.map(date => getWeekNumber(date));
};
...
const sendData = async () => {
  ...
  if (result)
    try {
      dataInstallment.data.date = formatDate(dateValues.value);
      dataInstallment.data.week_day = weekDays.value;
      dataInstallment.monthly_payment = parseFloat(dataAmount.value);
      dataInstallment.credit_sum = props.allSum;
...
watch(selectedTerm, (newVal) => {
  ...
  dataInstallment.credit_term = newVal;
...
<template>
  <div class="installment">
    ...
    <div class="flex ml-[220px] mt-2">
      <div class="text-green-600">Итого бонус: {{ props.allSum }}</div>
    </div>
