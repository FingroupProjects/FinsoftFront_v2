...
const copyProducts = async () => {
  ...
  try {
    const res = await useAxios(`/document/movement/copy/${id.value[0]}`, {
      method: "POST",
    });
    emit('getProduct')
...
const deleteProduct = async () => {
  ...
  const endpoint = props.selectProducts[0].deleted_at
      ? '/document/movement/massRestore'
      :'/document/movement/massDelete' ;
...
async function conductMethod(){
  ...
  const endpoint = props.selectProducts[0].active
      ? '/document/movement/unApprove'
      :'/document/movement/approve' ;
