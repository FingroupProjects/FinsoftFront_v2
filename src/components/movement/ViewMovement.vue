...
const getView = async () => {
  try {
    ...
    viewDocument.value = {
      ...
      author: item.author,
      recipientStorageName: item.recipient_storage_id,
      senderStorageName: item.sender_storage_id,
...
const updateView = async () => {
  ...
  if (result) {
    try {
      ...
      console.log(viewDocument.value)
      const data = {
        organization_id: viewDocument.value.organizationName.id || viewDocument.value.organizationName.code,
        recipient_storage_id: viewDocument.value.recipientStorageName.id || viewDocument.value.recipientStorageName.code,
        sender_storage_id: viewDocument.value.senderStorageName.id || viewDocument.value.senderStorageName.code,
...
const fetchExistingGoods = async () => {
  try {
    const response = await useAxios(`/document/movement/${props.productId}`);
...
const approve = async () => {
  try {
    const res = await useAxios(`/document/movement/approve`, {
      method: 'POST',
      data: {
        ids: [`${props.productId}`]
      }
...
const unApprove = async () => {
  try {
    const res = await useAxios(`/document/movement/unApprove`, {
      method: 'POST',
      data: {
        ids: [`${props.productId}`]
      }
...
<template>
  ...
  <div class="edit-purchase">
    ...
    <div class="header">
      <div class="flex gap-[16px] pt-2">
        <div>
          <div class="header-title">Перемещение</div>
    ...
    <div v-if="isOpen"
      ...
      <FloatLabel class="col-span-4">
        <Select v-model="viewDocument.senderStorageName" :options="storage" class="w-full" option-label="name"
                @click="findStorage">
          <template #value>
            {{ viewDocument.senderStorageName?.name }}
          </template>
        </Select>
        <label for="dd-city">Склад отправитель</label>
      </FloatLabel>
      <FloatLabel class="col-span-4">
        <Select v-model="viewDocument.recipientStorageName" :options="storage" class="w-full" option-label="name"
                @click="findStorage">
          <template #value>
            {{ viewDocument.recipientStorageName?.name }}
          </template>
        </Select>
  ...
  <movement-table :productId="productId" @post-goods="getProducts" @editModal="changeModal"/>
