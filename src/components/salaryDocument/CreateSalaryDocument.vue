...
async function getEmployeeSalary() {
  try {
    const res = await useAxios(`reportCard/employeeSalary`, {
      method: "GET",
      params: {
        organization_id: createValues.organization.code,
        month_id: createValues.month.code
  ...
  <div class="create-purchases">
    ...
    <div class="form grid grid-cols-12 gap-[16px] mt-[30px]">
      ...
      <FloatLabel class="col-span-12 mt-[10px]">
        <Textarea v-model="createValues.comment" class="w-full" style="min-height: 20px" rows="8" cols="10"/>
        <label for="dd-city">Комментарий</label>
      </FloatLabel>
    </div>
  </div>
  <EmployeesSalary @postGoods="getProducts" :employees="employees"/>
