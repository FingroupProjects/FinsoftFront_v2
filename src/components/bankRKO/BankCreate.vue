...
import BankTabs from "@/components/bankRKO/BankTabs.vue";
import BankPc from "@/components/bankRKO/BankPc.vue";
import BankTabsCass from "@/components/bankRKO/BankTabsCass.vue";
import BankInvestment from "@/components/bankRKO/BankInvestment.vue";
import BankCreditReceive from "@/components/bankRKO/BankCreditReceive.vue";
import BankProviderRefound from "@/components/bankRKO/BankProviderRefound.vue";
import BankAccountablePersonRefund from "@/components/bankRKO/BankAccountablePersonRefund.vue";
import BankOtherExpenses from "@/components/bankRKO/BankOtherExpenses.vue";
import BankOtherIncomes from "@/components/bankRKO/BankOtherIncomes.vue";
...
<template>
  <div>
    ...
    <div class="grid grid-cols-12 gap-[46px] mt-[30px]">
      ...
      <BankTabs v-if="activeTab?.code === 10" @close-sidebar="closeFn" @close-dialog="postMethod"
                   :operation-type="activeTab?.code" :type="activeTab.type"/>
      <BankPc v-if="activeTab?.code ===11" @close-sidebar="closeFn" @close-dialog="postMethod"
                 :operation-type="activeTab?.code" :type="activeTab.type"/>
      <BankTabsCass v-if="activeTab?.code ===12" @close-sidebar="closeFn" @close-dialog="postMethod"
                       :operation-type="activeTab?.code" :type="activeTab.type"/>
      <BankInvestment v-if="activeTab?.code ===13" @close-sidebar="closeFn" @close-dialog="postMethod"
                         :operation-type="activeTab?.code" :type="activeTab.type"/>
      <BankCreditReceive v-if="activeTab?.code ===14" @close-sidebar="closeFn" @close-dialog="postMethod"
                            :operation-type="activeTab?.code" :type="activeTab.type"/>
      <BankProviderRefound v-if="activeTab?.code ===15" @close-sidebar="closeFn" @close-dialog="postMethod"
                              :operation-type="activeTab?.code" :type="activeTab.type"/>
      <BankAccountablePersonRefund v-if="activeTab?.code ===16" @close-sidebar="closeFn"
                                         @close-dialog="postMethod" :operation-type="activeTab?.code"
                                         :type="activeTab.type"/>
      <BankOtherExpenses v-if="activeTab?.code ===17" @close-sidebar="closeFn" @close-dialog="postMethod"
                              :operation-type="activeTab?.code" :type="activeTab.type"/>
      <BankOtherIncomes v-if="activeTab?.code ===18" @close-sidebar="closeFn" @close-dialog="postMethod"
                             :operation-type="activeTab?.code" :type="activeTab.type"/>
